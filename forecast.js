...
const PY_FORECAST = [];
const LAT_RES = 5, LON_RES = 5, STEPS = 56;
const START_DATE = new Date(Date.UTC(2026,0,24,12));

const latitudes = []; for(let lat=-90; lat<=90; lat+=LAT_RES) latitudes.push(lat);
const longitudes = []; for(let lon=-180; lon<=180; lon+=LON_RES) longitudes.push(lon);

function randomVariation(value, factor=0.5){ return value + (Math.random()*2-1)*value*factor; }
...
function drawForecast(stepIndex){
    ...
    heatLayer = L.heatLayer(heatPoints,{radius:25,blur:15,gradient:buildGradient(variable)}).addTo(map);
    ...
    if(variable==='wind10' || variable==='gust1h'){
        data.forEach(d=>{
            const marker = L.marker([d.lat,d.lon],{
                icon:L.divIcon({className:'wind-arrow',html:'&#8593;',iconSize:[20,20]}),
...
// ---------------- CONTROLS ----------------
function nextStep(){ currentStep = (currentStep+1)%PY_FORECAST.length; drawForecast(currentStep); }
function prevStep(){ currentStep = (currentStep-1+PY_FORECAST.length)%PY_FORECAST.length; drawForecast(currentStep); }
